...
#wiresRaw = """R75,D30,R83,U83,L12,D49,R71,U7,L72
#U62,R66,U55,R34,D71,R55,D58,R83"""

#wiresRaw = """R8,U5,L5,D3
#U7,R6,D4,L4"""

wiresList = wiresRaw.splitlines()

wire1Raw = wiresList[0]
wire2Raw = wiresList[1]

wire1List = wire1Raw.split(',')
wire2List = wire2Raw.split(',')

def findMaxRange(wireList):
    maxX = -1
    minX = 1
    maxY = -1
    minY = 1
...
#print(findMaxRange(wire1List))
#print(findMaxRange(wire2List))
...
grid = [[0 for j in range(0, GRID_Y)] for i in range(0, GRID_X)]

# Center Point
grid[CENTER_X][CENTER_Y] = -1000

def manhattanDist(x, y):
    return abs(x - CENTER_X) + abs(y - CENTER_Y)

def traceWire(wireList):
    x = CENTER_X
    y = CENTER_Y
    touchedSet = set((x,y))
    for instruction in wireList:
        ...
        if(direction == 'R'):
            for i in range(x + 1, x + num + 1):
                if((i,y) not in touchedSet):
                    grid[i][y] += 1
                    touchedSet.add((i,y))
            x += num
        elif(direction == 'U'):
            for i in range(y + 1, y + num + 1):
                if((x,i) not in touchedSet):
                    grid[x][i] += 1
                    touchedSet.add((x,i))
            y += num
        elif(direction == 'L'):
...
for x in range(0, len(grid)):
    for y in range(0, len(grid)):
        if grid[x][y] > 1:
            minDist = min(minDist, manhattanDist(x, y))

print(minDist)

#for y in range(len(grid)-1, -1, -1):
#    print(''.join([grid[i][y] for i in range(0, len(grid))]))
