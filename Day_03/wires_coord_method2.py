...
for instruction in wire1List:
    direction = instruction[0]
    num = int(instruction[1:])
    if(direction == 'R'):
        for i in range(1, num + 1):
            touchedSet[(x + i,y)] = steps + i
        x += num
    elif(direction == 'U'):
        for i in range(1, num + 1):
            touchedSet[(x,y + i)] = steps + i
        y += num
    elif(direction == 'L'):
        for i in range(1, num + 1):
            touchedSet[(x - i,y)] = steps + i
...
y = 0
steps = 0
for instruction in wire2List:
    direction = instruction[0]
    num = int(instruction[1:])
    if(direction == 'R'):
        for i in range(1, num + 1):
            if((x + i, y) in touchedSet):
                intersections.append(touchedSet[(x + i, y)] + steps + i)
        x += num
    elif(direction == 'U'):
        for i in range(1, num + 1):
            if((x, y + i) in touchedSet):
                intersections.append(touchedSet[(x, y + i)] + steps + i)
        y += num
    elif(direction == 'L'):
        for i in range(1, num + 1):
            if((x - i,y) in touchedSet):
